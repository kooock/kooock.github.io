...
그렇다면 과연 변동성은 리스크니까 항상 나쁜 것만을 의미하는 것일까?

다음 예문을 한번 보자 변동은 있지만 기대수익이 0인 자산 A와 변동과 수익 둘 다 없는 자산 B가 존재하는 경우이다,

    또 다른 동전게임이 있다. 만약에 동전을 던져서 앞면이
    나오면 배팅한 그액이 두배로 불어나고, 반대로 뒷면이 
    나오면 배팅금액이 절반으로 줄어드는 룰이라고 하자. 
    그리고 매번 동전 던지기의 겨로가에 따라서 수익 배분을
     한다고 가정한다. 초기 투자 자본 $$x_{0}$$를 
     시작으로 플레이하려고 한다. 어떤 전략을 사용해야 
     하는가? 자본 증식은 가능한가?
     
먼저 자본금 전부를 배팅하는 경우를 생각해본다.

앞면이 나왔을 때 기대할 수 있는 자본금은 $$x_{1}=2x_{0}$$이다.

반면에 뒷면이 나왔을 떄 기대할 수 있는 자본금은 $$x_{1}={1 \over 2}x_{0}$$이다.

그렇다면 N번을 시행한다고 가정하고 확률적으로 앞면만 나왔을때의 자본 변화 만을 계산하면

$$x_{N \over 2} = 2^{N \over 2}x_{0}$$ 가된다.

반면에 확률적으로 뒷면만 나왔을때의 자본 변화 만을 계산하면

$$x_{N \over 2} = {1 \over 2}^{N \over 2}x_{0}$$ 가된다.

이 두 경우를 곱해주면 이 둘 모두를 고려한 자본의 기대값을 계산할 수 있는데

$$x_{N} = 2^{N \over 2}{1 \over 2}^{N \over 2}x_{0} = 2^{N \over 2}2^{-{N \over 2}}x_{0} = 2^{ {N \over 2} - {N \over 2}}x_{0} = 2^{0}x_{0}= x_{0}$$ 

결국엔 변동은 있지만 아무런 수익이 나지 않는다. 

자 그럼 배팅 비율을 a라고 치고 다시한번 배팅해보자

앞면이 나왔을때의 자본의 기대값은 $$x_{1} = 2ax_{0} + (1-a)x_{0} = (1+a)x_{0}$$ 이고,

앞면만 나올때의 기대값은 확률적으로 $${N \over 2}$$번 앞면이 나오게 되므로  $$x_{N \over 2} = (1+a)^{N \over 2}x_{0}$$이다. 

뒷면이 나왔을때의 자본의 기대값은 $$x_{1} = {1 \over 2}ax_{0} + (1-a)x_{0} = (1-{1 \over 2}a)x_{0}$$ 이다. 

앞면만 나올때의 기대값은 마찬가지로 확률적으로 $${N \over 2}$$번 앞면이 나오게 되므로  $$x_{N \over 2} = (1-{1 \over 2}a)^{N \over 2}x_{0}$$이다.

이 둘을 곱하면 최종 기댓값은 $$x_{N} = (1+a)^{N \over 2}(1-{1 \over 2}a)^{N \over 2}x_{0}$$가 된다.

자 그럼 이것을 위의 문제처럼 미분해보자
